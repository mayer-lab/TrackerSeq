...
bbduk.sh in=$datadir/$R1 in2=$datadir/$R2 k=17 literal=GACTCTGGCTCACAAAT  ktrim=r out=stdout.fq int=f skipr1 maq=15 overwrite=t|\
        bbduk.sh in=stdin.fq literal=CTGA k=4 restrictleft=4 ktrim=l out1=$output/trim_R1.fastq out2=$output/trim_R2.fastq outm1=$output/dis_R1.fastq outm2=$output/dis_R2.fastq int=t skipr1 overwrite=t

### 2. Remove short reads and re-pair
# Remove short reads and re-pair R1 and R2 (for libraries using piggybac)

bbduk.sh in=$output/trim_R2.fastq out=stdout.fq minlength=37 maxlength=37 overwrite=t| \
        repair.sh in1=stdin.fq in2=$output/trim_R1.fastq out1=$output/pair_R1.fastq out2=$output/pair_R2.fastq repair overwrite=t
