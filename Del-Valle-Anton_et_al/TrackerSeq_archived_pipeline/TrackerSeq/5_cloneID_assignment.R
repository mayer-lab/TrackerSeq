...
sparse_matrix1 <- readr::read_csv("sparse_matrix_leven_read10_umi9.csv")
dim(sparse_matrix1)
...
barcode_list1 <- read.table("barcode_list_leven_read10_umi9.txt")
head(barcode_list1)
...
saveRDS(g, paste0(out_path1, "cloneID_igraph_nread10_numi9.rds"))
...
write.csv(clones, file = paste0(out_path1, "cloneIDs_nreads10_numi9.csv"))

## add clonal IDs to seurat-object:
ferret_seurat <- readRDS("ferret_seurat.rds")

## in this case one biological sample was split across 4 lanes ##
...
## save seurat object containing cloneID annotation:
saveRDS(ferret_seurat, paste0(out_path1, "ferret_seurat_wLineage.rds"))

## script last updated on: 21.05.2023 ##
## author: Florian Neuhaus (florian.neuhaus@bi.mpg.de) ##